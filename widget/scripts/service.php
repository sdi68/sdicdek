...
class ISDEKservice
{
	...
	protected static function getSDICDEK(&$sc)
	{

		$sdicdek_file = $_SERVER['DOCUMENT_ROOT'] . '/includes/modules/shipping/sdicdek/sdicdek.php';

		if (file_exists($sdicdek_file))
		{
	...
	protected static function calculate($shipment)
	{
		...
		if(($sc !== null && $sc instanceof sdicdek) && (!isset($shipment['tariffId']) || $shipment['tariffId'] == false)) {
			// SDI получаем тариф из модуля
			$shipment['tariffId'] = $sc->getTarif();
		}
	...
	protected static function getHeaders()
	{
		...
		// SDI инициализируем класс sdicdek чтоб получить настройки модуля
		self::getSDICDEK($sc);
		if(($sc !== null && $sc instanceof sdicdek) && (self::$account === false || self::$key === false)){
			// SDI получаем учетные данные из модуля
			self::$account = $sc->getAccount();
			self::$key = $sc->getSecure();
		}
